...
//Necesario para retroachievements
...
/* ------------------------------------------------------------------ */
/* RC_INLINE – portable inline keyword                                 */
/* ------------------------------------------------------------------ */
#if defined(_MSC_VER)
  #define RC_INLINE __forceinline
#elif defined(__GNUC__) || defined(__clang__)
  #define RC_INLINE __attribute__((always_inline)) static inline
#else
  #define RC_INLINE static inline
#endif

/* ------------------------------------------------------------------ */
/* Include endianness utilities (rc_endian.h).                         */
/* This header is self-contained and safe to include everywhere.       */
/* ------------------------------------------------------------------ */
#include "rc_endian.h"
...
#define RC_NO_VARIADIC_MACROS 1
 ...
 #define rc_mutex_unlock(mutex)
  ...
  /* Xbox 360 (XDK) – uses Win32-compatible CRITICAL_SECTION */
  typedef struct rc_mutex_t {
    CRITICAL_SECTION critical_section;
  } rc_mutex_t;
 #elif defined(GEKKO)
