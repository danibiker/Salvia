...
typedef struct {
	 ...
	 * hardware's 1-cycle load delay, which Soul Reaver / Soul Calibur
	 * and similarly tight code rely on. dloadReg[i] is the destination
	 * GPR (0 = empty); dloadVal[i] is the value waiting to be committed.
	 * The two slots alternate so two consecutive loads can be in flight. */
	u32 dloadReg[2];
	u32 dloadVal[2];
} psxRegisters;
