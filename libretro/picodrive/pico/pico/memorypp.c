...
#include <platform/common/input_pico.h>
   ...
   #include <time.h>
   /* Windows/Xbox no tienen sys/time.h, si necesitas estructuras de tiempo 
      frecuentemente se definen aquí o se usa la API de Windows */
#else
   #include <sys/time.h>
#endif
...
#if defined(_WIN32) || defined(_XBOX)
#ifdef _XBOX
	#include <xtl.h> // Para Xbox
#else
	#include <windows.h> // Para Windows PC
#endif

u64 get_ticks(void)
{
    // GetTickCount devuelve milisegundos desde que se inició el sistema
    return (u64)GetTickCount();
}
#else
// Mantener la versión original para Linux/LibXenon si fuera necesario
u64 get_ticks(void)
{
    struct timeval tv;
