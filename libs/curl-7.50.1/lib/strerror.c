...
{
  /* 1. DECLARACIONES AL PRINCIPIO (C89) */
  static char xbox_err[64];
  int real_err;
  char *buf, *p;
  size_t max;
  int old_errno = ERRNO;

  /* 2. LOGICA ESPECIFICA DE XBOX */
#ifdef _XBOX
  (void)conn;
  (void)err;
  /* Obtenemos el error real del socket de Xbox */
  real_err = (int)WSAGetLastError();
  /* Usamos la version de cURL para evitar el warning de 'sprintf' */
  curl_msnprintf(xbox_err, sizeof(xbox_err), "Xbox Net Error: %d", real_err);
  return xbox_err;
#else
  /* 3. LOGICA ORIGINAL (PARA OTROS SISTEMAS) */
  DEBUGASSERT(conn);
  DEBUGASSERT(err >= 0);
  ...
  *buf = '\0';
  ...
  buf[max] = '\0';
  if((p = strrchr(buf, '\n')) != NULL && (p - buf) >= 2) *p = '\0';
  if((p = strrchr(buf, '\r')) != NULL && (p - buf) >= 1) *p = '\0';
