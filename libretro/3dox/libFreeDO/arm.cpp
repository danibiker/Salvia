...
//------------------------------ SWI HLE (toggle) --------------------------------------
// Ver swi_hle.h. HLE de los SWI de matematica de punto fijo del BIOS (0x50000..0x50012):
// ejecuta el equivalente nativo en C en vez de interpretar el codigo ARM del BIOS.
// Por defecto OFF; se activa con _arm_SetSWIHLE() (via FDP_SET_SWI_HLE).
int g_swi_hle = 0;
...
// Devuelve true si el SWI se ha resuelto por HLE (no hay que vectorar al BIOS).
// pRam es host-native (big-endian en Xbox360) = como lo ve el ARM del 3DO; los
// punteros vec/mat se asumen alineados a 4 (ABI matematico del 3DO).
static bool decode_swi_hle(unsigned int cmd)
{
    uint8  *ram = pRam;
    uint32  r0  = RON_USER[0];
    uint32  r1  = RON_USER[1];
    uint32  r2  = RON_USER[2];
    uint32  r3  = RON_USER[3];

    switch(cmd & 0x000FFFFF)
    {
    case 0x50000: MulVec3Mat33_F16(*(vec3f16*)&ram[r0], *(vec3f16*)&ram[r1], *(mat33f16*)&ram[r2]); return true;
    case 0x50001: MulMat33Mat33_F16(*(mat33f16*)&ram[r0], *(mat33f16*)&ram[r1], *(mat33f16*)&ram[r2]); return true;
    case 0x50002: MulManyVec3Mat33_F16((vec3f16*)&ram[r0], (vec3f16*)&ram[r1], *(mat33f16*)&ram[r2], (int32)r3); return true;
    case 0x50005: MulManyF16((frac16*)&ram[r0], (frac16*)&ram[r1], (frac16*)&ram[r2], (int32)r3); return true;
    case 0x50006: MulScalerF16((frac16*)&ram[r0], (frac16*)&ram[r1], (frac16)r2, (int32)r3); return true;
    case 0x50007: MulVec4Mat44_F16(*(vec4f16*)&ram[r0], *(vec4f16*)&ram[r1], *(mat44f16*)&ram[r2]); return true;
    case 0x50008: MulMat44Mat44_F16(*(mat44f16*)&ram[r0], *(mat44f16*)&ram[r1], *(mat44f16*)&ram[r2]); return true;
    case 0x50009: MulManyVec4Mat44_F16((vec4f16*)&ram[r0], (vec4f16*)&ram[r1], *(mat44f16*)&ram[r2], (int32)r3); return true;
    case 0x5000C: RON_USER[0] = (uint32)Dot3_F16(*(vec3f16*)&ram[r0], *(vec3f16*)&ram[r1]); return true;
    case 0x5000D: RON_USER[0] = (uint32)Dot4_F16(*(vec4f16*)&ram[r0], *(vec4f16*)&ram[r1]); return true;
    case 0x5000E: Cross3_F16(*(vec3f16*)&ram[r0], *(vec3f16*)&ram[r1], *(vec3f16*)&ram[r2]); return true;
    case 0x5000F: RON_USER[0] = (uint32)AbsVec3_F16(*(vec3f16*)&ram[r0]); return true;
    case 0x50010: RON_USER[0] = (uint32)AbsVec4_F16(*(vec4f16*)&ram[r0]); return true;
    case 0x50011: MulVec3Mat33DivZ_F16(*(vec3f16*)&ram[r0], *(vec3f16*)&ram[r1], *(mat33f16*)&ram[r2], (frac16)r3); return true;
    case 0x50012:
        {
            vec3f16  *dest  = (vec3f16*) &ram[*(uint32*)&ram[r0 + 0x00]];
            vec3f16  *src   = (vec3f16*) &ram[*(uint32*)&ram[r0 + 0x04]];
            mat33f16 *mat   = (mat33f16*)&ram[*(uint32*)&ram[r0 + 0x08]];
            frac16    n     = *(frac16*)&ram[r0 + 0x0C];
            uint32    count = *(uint32*)&ram[r0 + 0x10];
...
__inline void ARM_SET_C(uint32 x)
{
    //old_C=(CPSR>>29)&1;
...
/* inlinado (opera lo marca INLINE): un unico call-site (arm.cpp ALU),
   quitado __fastcall (x86-only, ignorado en PPC/XDK) */
__forceinline bool ARM_ALU_Exec(uint32 inst, uint8 opc, uint32 op1, uint32 op2, uint32 *Rd)
{
 switch(opc)
 {
   case 0:
...
__forceinline unsigned int calcbits(unsigned int num)
{
 unsigned int retval;
...
int _arm_Execute()
{
   ...
   int ssss=((cond_flags_cross[(((uint32)cmd)>>28)]>>((CPSR)>>28))&1);
    ...
    Undefine:
			...
			case 0xf:	//SWI
					if(g_swi_hle && decode_swi_hle(cmd))
						CYCLES-=SCYCLE+NCYCLE;   // coste del SWI (2S+1N), como opera; sin vectorar al BIOS
					else
						decode_swi(cmd);         // LLE: vectora a 0x08 (ya cobra 2S+1N)
					break;
			//---------
                        default:	//coprocessor
					//!!Exeption!!
