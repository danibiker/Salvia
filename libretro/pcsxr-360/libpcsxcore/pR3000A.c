 ...
 * change psxRegs.GPR.r[reg] in memory. We use iFlushReg so anything
 * dirty in the cache is written back FIRST — that matters because the
 * delay slot of a load can read this register and must see the value
 * the GAME left there before the load (e.g. an earlier ALU's result),
 * NOT a stale memory value. iFlushReg also handles the constant-
 * propagation case (ST_CONST without ST_MAPPED) by materialising the
 * constant into memory before clearing the tracker. */
static void dl_invalidate_iregs(int reg)
{
	if (reg <= 0) return;
	iFlushReg(0, reg);
	/* iFlushReg leaves state cleared via FlushHWReg; defensively make
	 * sure ST_CONST is also gone in case the reg was neither mapped
	 * nor a known constant (state already ST_UNK — no-op). */
	iRegs[reg].state = ST_UNK;
}
...
static void preMemRead()
{
	...
	if (_Rs_ != _Rt_) {
		 ...
		 * read it back from psxRegs.GPR.r[_Rt_]. iFlushReg writes back
		 * dirty mappings, materialises constants, and clears the
		 * tracker — exactly what we need. */
		iFlushReg(0, _Rt_);
#else
