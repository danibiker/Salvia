...
void Sync::init_fps_counter(double gameFps){
	if (gameFps > 0){
        this->fps = gameFps;
        // Mantenemos el frameDelay como double para el limitador de alta precisi√≥n
        this->frameDelay = 1000.0 / gameFps; 
        
        // Para los promedios (que parecen usar enteros), usamos el redondeo m√°s cercano
        initAverages((uint32_t)(frameDelay + 0.5));
    } else {
        // Fallback por si gameFps es inv√°lido
...
void Sync::update_fps_counter(bool updateFpsOverlay) {
    ...
    if (g_lastFrameTick == 0) g_lastFrameTick = currentTick; // Inicializaci√≥n en el primer uso
    
	// Calculamos cu√°nto tiempo ha pasado realmente desde el frame anterior
	...
	if (updateFpsOverlay){
		...
		// Calculamos la media (evitando divisi√≥n por cero)
		if (totalTime > 0) {
...
void Sync::limit_fps(double& nextFrameTime) {
    ...
    // --- MÈtricas de utilizaciÛn basadas en tiempo real de trabajo ---
    // workTime = tiempo que la CPU estuvo ocupada procesando el frame
    //          = desde que terminÛ el sleep del frame anterior hasta ahora
    if (lastWorkEnd > 0.0) {
        double workTime = currentTime - lastWorkEnd;
        double currentUtilization = (workTime / this->frameDelay) * 100.0;

        // Clamp: 0% = idle total, 100% = justo al lÌmite, >100% = no llega a tiempo
    ...
    if (diffTime > 0) {
        ...
        // SDL_Delay es seguro aquÌ porque el Busy Wait corregir· su imprecisiÛn
        if (diffTime > 4.0) {
        ...
        // Si hay un lag masivo, reseteamos para evitar el efecto "c√°mara r√°pida"
        nextFrameTime = currentTime;
    }

    // Registramos el instante en que terminamos de esperar (= inicio del trabajo del prÔøΩximo frame)
